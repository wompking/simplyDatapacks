...
execute as @e[type=minecraft:item_frame,tag=craftConveyorSpatula] at @s run data modify entity @s Item set value {id:"minecraft:carrot_on_a_stick",Count:1b,tag:{display:{Name:'{"text":"Plate Removing Spatula","italic":false}',Lore:['{"text":"Use this to remove things off the ground.","color":"gray","italic":true}','{"text":"Not safe for cooking.","color":"gray","italic":true}']},Unbreakable:1b,CustomModelData:3,CustomItemId:"simply:spatula",clickEvent:"simply_spatula"}}
